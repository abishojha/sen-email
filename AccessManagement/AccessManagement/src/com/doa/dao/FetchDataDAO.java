...
public class FetchDataDAO {
	...
	try {
		...
		SQLQuery query = session
				.createSQLQuery("SELECT branchId,branchName from branch");	
		for (Object[] row : (List<Object[]>) query.list()) {
			
			Branch branches = new Branch();		
			branches.setId((Integer) row[0]);
			branches.setBranch((String) row[1]);
		...
		logger.log(Level.WARNING, "ERROR IN DAO fetchBranch()");
		if (tx != null)
			tx.rollback();
		e.printStackTrace();
	} finally {
	...
	public List<UserAccess> fetchUserAccess() {		
		...
		Transaction tx = null;		
			try {
				tx = session.beginTransaction();
				SQLQuery query = session
						.createSQLQuery("SELECT b.branchName,a.userName,a.cbsId,a.jobTitle,a.cbsAccess,a.remarks,a.status FROM useraccess a inner join branch b ON a.branchCode=b.branchId order by b.branchName ASC");	
				for (Object[] row : (List<Object[]>) query.list()) {					
