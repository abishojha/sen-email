...
$("#updateAccess").click(function(){
	//$("#addAccess").prop('disabled', 'disabled');
	var url="updateUserAccess";	    		
	var postinsert = $.post(url, updateAccess());
    postinsert.done(function(data) {	
	alertresponse(data);
   window.location.reload(true);    		
...
function updateAccess(){
 var updateUserAccess={
			updBranch:$("#updBranch").val(),
			updAccName:$("#updAccName").val(),
			updcbsId:$("#updcbsId").val(),
			updTitle:$("#updTitle option:selected").text(),
			updgrpAcc:$("#updTitle").val(),	
...
function updmyFunction() {
    var x = document.getElementById("updTitle").value;
    document.getElementById("updgrpAcc").innerHTML =x;	    
}
